...
RSpec.describe Activity, type: :model do
  context 'correctly associates between a user and activities as a creator, and groups' do
    ...
    it 'check all types activities' do
      gr = user.groups.create!(name: "lalala", icon: "icon")
      user.create_external_activity(name: "Group Activity", description: "New Description", schedule_date: Time.now, group_id: gr.id)
    ...
    it 'check associations between a group and a group activity' do
      gr = user.groups.create!(name: "lalala2", icon: "icon2")
      act = user.create_external_activity(name: "Group Activity", description: "New Description", schedule_date: Time.now, group_id: gr.id)
    ...
    it 'create a group activity with a wrong length of the title' do
      gr = user.groups.create(name: "new group", icon: "icon2")
      act = user.create_external_activity(name: "G", description: "New Description", schedule_date: Time.now, group_id: gr.id)
